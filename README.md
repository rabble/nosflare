...
Nosflare can require that all events include a valid `client` tag to ensure events only come from approved applications. This is enabled by default with `REQUIRE_CLIENT_TAG = true` in the `src/config.ts` file. Events must include a tag like `["client", "divine.video"]` where the value matches one of the allowed clients in the `allowedClientTags` set. Valid client identifiers by default are: `divine.video`, `divine-web`, `divine`, and `openvine`. Events without a valid client tag are rejected with a user-friendly message directing them to use an official app. This helps maintain quality control and reduces spam from unauthorized clients. Set `REQUIRE_CLIENT_TAG` to `false` to disable this feature. See [docs/CLIENT_TAG_VALIDATION.md](/docs/CLIENT_TAG_VALIDATION.md) for complete documentation.
